...
char *punct_short[]={",",".",";",":","'","\"","?","!","''"};
char *punct_long[]={"comma","full_stop","semi_colon","colon","single_quote","double_quote","question_mark","exclamation","2_single_quotes"};
...
int get_punct_full(char *ps)
{
  ...
  for(i=0;i<8;i++)
    {
      if(strncmp(ps,punct_short[i],1)==0)
	return i;
    }
  if(strncmp(ps,punct_short[8],2)==0) return 8;
  return -1;
}
...
int main(int argc,char **argv)
{
  ...
  FILE  *root_word_picked_up,*tolower_word_clp,*paxa_clp,*all_facts,*eng_fp;
  int   i=0,j,index,flag,map[MAX_NO_OF_LINES][2],lag,file_end,wrd_len=0,first_cap_flag=0,all_cap_flag=0;
  char  temp_left_punct[5],temp_right_punct[5],*temp_ptr,*str_tolower,str1[100];
  ...
  while(1)
      ...
      if(strcmp(para_no,prev_para)!=0 || strcmp(line_no,prev_line)!=0 ||file_end)
	{
	  ...
	  mkdir(filename,0777);
          ...
          sprintf(filename,"%s/%s_tmp/%s.%s",argv[1],argv[2],prev_para,prev_line);
          sprintf(filename,"%s/all_facts",filename);
          all_facts=fopen(filename,"w");
          if(all_facts==NULL) {printf("Could not open all_facts for writing\n");exit(1);}
          fprintf(all_facts,"(para_id-sent_id-no_of_words\t%s\t%s\t%d)\n",prev_para,prev_line,index-1);
