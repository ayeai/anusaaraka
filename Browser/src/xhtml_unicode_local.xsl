...
<link href="english_hindi.css" type="text/css" rel="stylesheet" />
<script src="script.js" type="text/javascript"></script>
<script src="open.js" type="text/javascript"></script>
<script src="english_hindi.js" type="text/javascript"></script>
...
&lt;tr class="row</xsl:text>
    ...
    <td class="{$column_type}">
      <xsl:variable name="node_value" select="node()" />
	...
	<xsl:if test="$row_no = $no_of_rows">
	  ...
	  <xsl:if test="$column_no = 1">
	    <td class="number"><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text></td>
          ...
          <!-- Copy the contents of the last row to the suggestion row -->
          <xsl:variable name="node_value" select="node()" />
          <xsl:call-template name="handle_whitespace">
            <xsl:with-param name="word" select="." />
          </xsl:call-template>
	  <td class="{$column_type}"><input name="suggestion_{$sentence_name}.{$column_no}" type="text" class="suggestion" size="1" value="{$node_value}"/>
</td></tr>
	  <xsl:text disable-output-escaping="yes">
	    &lt;/table&gt;</xsl:text>
	</xsl:if>
