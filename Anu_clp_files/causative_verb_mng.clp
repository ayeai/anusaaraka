...
;  =>
; (assert (id-HM-source 10000 jo ?src))
; )
 ...
 =>
	...
	(if (eq ?a "FALSE") then
		...
		(printout ?*caus_mng-file* "(id-HM-source   "?causative_vrb_id"   "?a "   " ?src ")" crlf)
                (printout ?*caus_mng-file* "(id-HM-source   "?vrb_id"    - " ?src ")" crlf)
		(retract ?mng ?mng1)
	)
  )
...
;Ex. She was asked about the pay increase but made no comment.
 ...
 (id-HM-source  10001 mng_to_be_decided_for_inserted_sub  ?id)
  =>
                (printout ?*caus_mng-file* "(id-HM-source   10001   "?h_mng "    FROM_SUB)" crlf)
  )
