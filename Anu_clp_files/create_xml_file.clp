...
(string-to-field (sub-string 2 10000 ?link_id)))
 ...
 ;(defrule test_for_complete_sen
 ;(declare (salience 3002))
 ;(id-word ?id ?word)
 ;(id-HM-source   ?  ?hmng   Complete_sen_gdbm)
 ;(test (< ?id ?id1))
 ;=>
 ;(assert (id-Apertium_output ?id -))
 ;)

 ;(defrule test_for_complete_sen1
 ;(declare (salience 3001))
 ;?f<-(id-Apertium_output ?id -)
 ;(id-HM-source   ?id  ?hmng   Complete_sen_gdbm)
 ;=>
 ;(retract ?f)
 ;(assert (id-Apertium_output ?id ?hmng))
 ;)
...
;(defrule test_padasuthra_for_proper_nouns
;(declare (salience 2001))
;?f<-(id-padasuthra ?id PropN)
...
;(defrule test_hindi_mng_for_proper_nouns
;(declare (salience 2001))
;?f<-(id-HM-source ?id ?mng PropN)
...
;(defrule Apertium_output_for_proper_nouns
;(declare (salience 2001))
;?f<-(id-Apertium_output ?id $?Apertium_output)
;(id-cat_coarse ?id PropN)
;(id-original_word ?id ?word)
;(not(pada_info (group_head_id ?id)(vibakthi ?vib&~0)))
;(not (id_Apertium_output_mng_modified ?id))
;=>
;(retract ?f)
;(assert (id-Apertium_output ?id ?word))
...
(printout fp "<english category='none'><a onclick=\"openwindow('"?word"','"?Eng_sen"')\">" ?l_punc ?original_word ?r_punc"</a> </english>" crlf)
...
(printout fp "<english number='"?p_id"."?s_id"' category='none'> <a onclick=\"openwindow('"?word"','"?Eng_sen"')\">" ?l_punc ?original_word ?r_punc"</a> </english>" crlf)
(assert (id-index 1 ?n_words))
)
