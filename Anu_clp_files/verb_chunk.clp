 ...
 (link_name-link_lnode-link_rnode ?TO   ?Pvr_node  ?TOr_node)
 ...
 (linkid-word-node_cat ?TOr_node ?TOword ?)
 (linkid-word-node_cat ?I2r_node ?I2word ?)
 (parser_id-root ?I2r_node ?root)
 =>
  (assert (G5_I_id_to_be_retracted ?sl_node))
  (printout lwg_fp "(root-verbchunk-tam-parser_chunkids  " ?root " " ?Iword"_"?Pvword"_"?TOword"_"?I2word"  "?Iword"_"?Pvword"_"?TOword"_0  "  ?Irnode " " ?Pvr_node " "?TOr_node" "?I2r_node ")" crlf)
  (assert (grouped_id ?sl_node)) 
  (printout ?*lwg_debug_file* "(rule_name-grouped_ids  TS_I_Pv_TO_I_rule " ?Irnode " "?Pvr_node " "?TOr_node" "?I2r_node ")"crlf)
 )
...
; ----------------------------------------- ( S  PP TO  I   Pv) (suf - en) -----------------------------------------------
 ...
 (linkid-word-node_cat  ?TOl_node had|have|need|was|were|be ?)
...
; -----------------------------------------  ( S I TO I ) (suf - 0) -----------------------------------------------------
 ...
 (linkid-word-node_cat  ?sr_node ?sword ?)
 (linkid-word-node_cat  ?Ir_node  ?Iword ?)
 (linkid-word-node_cat  ?TOl_node had|have|need|are|is|was|were|be ?)
 (linkid-word-node_cat  ?TOr_node ?Tword ?)
 (linkid-word-node_cat  ?I2r_node  ?I2word ?)
 (parser_id-root ?I2r_node ?root)
 =>
...
; -----------------------------------------  ( S I PP Pg ) (suf - ing) -----------------------------------------------------
 ...
 (linkid-word-node_cat  ?TOl_node able|unable ?)
 (linkid-word-node_cat  ?TOr_node ?Tword ?)
 (linkid-word-node_cat  ?Ir_node  ?Iword ?)
 (parser_id-root ?Ir_node ?root)
 =>
  (assert (G4_NI_id_to_be_retracted ?sr_node))
  (printout lwg_fp  "(root-verbchunk-tam-parser_chunkids " ?root " "?sword"_able_"?Tword"_"?Iword "  "?sword"_able_"?Tword"_0  " ?sr_node " "?Par_node " "?TOr_node " "?Ir_node  ")" crlf)
...
;------------------------------------------- ( S  I  Pv ) (suf - en) ---------------------------------------------------
 ...
 (link_name-link_lnode-link_rnode ?I    ?TOr_node ?Ir_node)
 (link_name-link_lnode-link_rnode ?TO  ?TOl_node ?TOr_node)
 (linkid-word-node_cat   ?TOl_node had|have|need ?)
 (linkid-word-node_cat   ?sr_node ?sword ?)
 (linkid-word-node_cat   ?Ir_node ?Iword ?)
 (linkid-word-node_cat   ?TOr_node ?Tword ?)
 (parser_id-root ?Ir_node ?root)
...
;(link_name-link_lnode-link_rnode ?O    ?let  ?us)
;(link_name-link_lnode-link_rnode I ?let ?kriyA)
;(linkid-word-node_cat  ?us us ?)
;(linkid-word-node_cat  ?kriyA ?word ?)
;(parser_id-root  ?let  let)
;(parser_id-root  ?kriyA  ?root)
; =>
; (assert (G3_NI_id_to_be_retracted ?let))
; (printout lwg_fp "(root-verbchunk-tam-parser_chunkids "?root "  let_us_"?word "  "  let_us_0" " ?let" "?us" "?kriyA")" crlf)
; (assert (grouped_id ?let ));(assert (grouped_id ?us)) (assert (grouped_id ?kriyA))
; (assert (grouped_id ?kriyA))
; (printout ?*lwg_debug_file* "(rule_name-grouped_ids  Wi_O_let_us  " ?let" "?us" "?kriyA")" crlf)
; )
...
;Added by Sukhada
 ...
 =>
  ...
  (printout ?*lwg_debug_file* "(rule_name-grouped_ids  SI_Pg_rule "?sl_node " " ?Pgr_node  ")" crlf)
 )
