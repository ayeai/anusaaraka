...
; (viSeRya-det_viSeRaNa  ?vi ?paxa_head)
 ?f3<-(to_be_included_in_paxa ?paxa_head)
...
; Ex. A man , apparently IN A BAD MOOD, was there. The girls IN THE LAB are tired. We did it in early December. The President announced on Monday that several more bases would be closed. The Zongle of Bongle Dongle resigned today. Is that the film in which he kills his mother.
 (defrule prep-noun
 (declare (salience 449))
 (link_name-lnode-rnode   Js|Jp|J|JG|Jw|IN|ON       ?prep   ?paxa_head)
...
=>
	...
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	prep-noun	P"?paxa_head	"	PP	"(implode$	?ids)	")"crlf)	
 )
...
;        (bind ?ids (create$ ))
 ...
 ;       )
	...
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	prep-noun2	P"?paxa_head	"	PP	"	?x	"	"?paxa_head	")"crlf)	
 )
...
=>
	...
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	stranded_prep-noun	P"?paxa_head	"	PP	"	?paxa_head	")"crlf)	
 )
...
=>
	...
	(bind ?count (+ ?count 1))
        (assert (count ?count))
	(assert (pada_info (group_head_id ?paxa_head) (group_cat PP) (group_ids ?ids) (preposition ?prep)))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	stranded_prep-noun1	P"?paxa_head	"	PP	"(implode$	(create$	?ids))	")"crlf)	
 )
...
(defrule RaRTI_viSeRaNa_rul_conj_and_or
 ...
 =>
 (retract ?f ?f0)
 (print_in_ctrl_fact_files  (nth$  (length $?grp_ids) $?grp_ids))
 ...
 (assert (pada_info (group_head_id ?head) (group_cat PP) (group_ids  ?pada_with_and) ))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	RaRTI_viSeRaNa_rul_conj_and_or	P"?head"	PP	"(implode$	?pada_with_and)	")"crlf)	
 )
 (bind ?count (+ ?count 2))
 (assert (count ?count))
 ...
 (assert (pada_info (group_head_id ?viSeRya) (group_cat PP) (group_ids  ?viSeRya) ))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	RaRTI_viSeRaNa_rule11	P"?viSeRya"	PP	"?viSeRya")"crlf)	
 )
 (assert (has_been_included_in_paxa ?viSeRya))
 (bind ?count (+ ?count 1))
 (assert (count ?count))
...
;Ex. of samAsa: This is my first sentence.
 ...
 ?f0<-(viSeRya-RaRTI_viSeRaNa   ?viSeRya  ?RaRTI_viSeRaNa)
 (or (viSeRya-viSeRaNa ?viSeRya ?viSeRaNa)(viSeRya-det_viSeRaNa ?viSeRya ?viSeRaNa) (samAsa  ?viSeRya ?viSeRaNa))
...
=>
	...
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	whose	P"?paxa_head"	English_PP	"(implode$	?a)	")"crlf)	
 )
...
=>
	...
	(retract ?f2 ?f3 ?f4)
        ...
        (bind ?b (delete$  ?a  1  1))
        (assert (pada_info (group_head_id ?paxa_head) (group_cat English_PP) (group_ids   ?a) (preposition ?prep)))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	prep_RaRTI_viSeRaNa	P"?paxa_head"	English_PP	"(implode$	?a)	")"crlf)	
 )
...
; Ex. I saw THE COWS.
 ...
 (or (kriyA-subject  ?verb ?paxa_head) (kriyA-dummy_subject   ?verb ?paxa_head) (kriyA-GNP_agr_subject   ?verb ?paxa_head))
 ?f1<-(to_be_included_in_paxa ?paxa_head)
 ?f<-(count ?count)
 (not (No complete linkages found)) 
...
; Ex. A man , apparently in a bad mood , was THERE.
 (defrule lupwa_prep
 (kriyA-lupwa_prep_saMbanXI  ?kriyA ?paxa_head)
 ?f0<-(to_be_included_in_paxa ?paxa_head)
 ?f<-(count ?count)
=>
	(print_in_ctrl_fact_files  ?paxa_head)
...
;EX. From in back of the shed I heard a scream that seemed to come from on top of the garage
 ...
 =>
        ...
        (assert (count ?count))
        (assert (has_been_included_in_paxa ?paxa_head))
        (assert (pada_info (group_head_id ?paxa_head) (group_cat PP) (group_ids ?paxa_head))(preposition ?prep1"_"?prep) )
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	idiom_type_3	P"?paxa_head	"	PP	"	?paxa_head	"	"?prep1"_"?prep	")"	crlf)	
)
...
=>
	...
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	MVpn-Ye	P"?paxa_head	"	PP	"?g")"crlf)	
...
;Ex. Rama and Mohan went to the shop. Rama, Mohan, Shyam  and Gita went to the school.
 (defrule and_sub
 (declare (salience 501))
 ?f2<- (kriyA-subject  ?kriya ?sub)
 (not (kriyA-subject  ?kriya ?sub_id1&:(< ?sub_id1 ?sub)))
 ?f3<- (kriyA-subject  ?kriya ?paxa_head)
 (not (kriyA-subject  ?kriya ?paxa_head1&:(> ?paxa_head1 ?paxa_head)))
 ?f0<-(to_be_included_in_paxa ?sub)
 ?f1<-(to_be_included_in_paxa ?paxa_head)
 ?f<-(count ?count)
 (test (< ?sub ?paxa_head))
 (not (has_been_included_in_paxa $?pre ?sub $?post))
=>
        (print_in_ctrl_fact_files  ?paxa_head)
        (retract ?f ?f0 ?f1 ?f2 ?f3)
        (bind $?ids (create$ ))
        (loop-for-count (?loop  ?sub  ?paxa_head)do
                (bind $?ids (insert$ $?ids 1 ?loop))
                (bind $?ids (sort > $?ids))
        )
        (assert (has_been_included_in_paxa $?ids))
        ...
        (assert (count ?count))
        (assert (pada_info (group_head_id ?paxa_head) (group_cat PP) (group_ids  $?ids) ))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	and_sub	P"?paxa_head	"	PP	"(implode$	(create$	$?ids	))	")"crlf)	
 )

;Ex. Then one day a man and woman came to the barn. My dog cat horse and mouse and his cow left.
 (defrule det_and_sub
 (declare (salience 509))
 (or (viSeRya-det_viSeRaNa ?sub ?det) (viSeRya-RaRTI_viSeRaNa ?sub ?det))
 ?f2<- (kriyA-subject  ?kriya ?sub)
 (not (kriyA-subject  ?kriya ?sub_id1&:(< ?sub_id1 ?sub)))
 ?f3<- (kriyA-subject  ?kriya ?paxa_head)
 (not (kriyA-subject  ?kriya ?paxa_head1&:(> ?paxa_head1 ?paxa_head)))
 ?f0<-(to_be_included_in_paxa ?sub)
 ?f4<-(to_be_included_in_paxa ?det)
 ?f1<-(to_be_included_in_paxa ?paxa_head)
 ?f<-(count ?count)
 (test (< ?sub ?paxa_head))
 (not (has_been_included_in_paxa $?pre ?sub $?post))
=>
        ...
        (assert (count ?count))
        (assert (pada_info (group_head_id ?paxa_head) (group_cat PP) (group_ids  $?ids) ))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	det_and_sub	P"?paxa_head	"	PP	"(implode$	(create$	$?ids	))	")"crlf)	
 )

  ;Ex. They were discussing their hopes and dreams.
 (defrule R_vi_and_obj
 (declare (salience 559))
 (viSeRya-RaRTI_viSeRaNa  ?ob ?R_vi)
 (kriyA-object  ?kriya ?ob)
 (not (kriyA-object  ?kriya ?ob_id1&:(< ?ob_id1 ?ob)))
 (kriyA-object  ?kriya ?paxa_head)
 (not (kriyA-object  ?kriya ?paxa_head1&:(> ?paxa_head1 ?paxa_head)))
 ?f0<-(to_be_included_in_paxa ?ob)
 ?f4<-(to_be_included_in_paxa ?R_vi)
 ?f1<-(to_be_included_in_paxa ?paxa_head)
 (test (< ?ob ?paxa_head))
 (not (has_been_included_in_paxa $?pre ?ob $?post))
 =>
        (print_in_ctrl_fact_files  ?paxa_head)
        (print_in_ctrl_fact_files  ?R_vi)
        (retract ?f1 ?f0 ?f4)
        (bind ?g (create$ ))
        (loop-for-count (?loop (+ ?R_vi 1) ?paxa_head )do
                (bind ?g (insert$ ?g 1 ?loop))
        )
        (bind ?a (sort > ?g))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	R_vi_and_obj	P"?paxa_head"	English_PP	"(implode$	(create$	?R_vi	?a))	")"	crlf)	
        (assert (pada_info (group_head_id ?paxa_head) (group_cat English_PP) (group_ids ?R_vi  ?a)))
  )

 ;Ex. My dog cat horse and mouse and his cow left.
 (defrule R_vi_and_sub
 (declare (salience 519))
 (viSeRya-RaRTI_viSeRaNa  ?sub ?R_vi)
 (kriyA-subject  ?kriya ?sub)
 (not (kriyA-subject  ?kriya ?sub_id1&:(< ?sub_id1 ?sub)))
 (kriyA-subject  ?kriya ?paxa_head)
 (not (kriyA-subject  ?kriya ?paxa_head1&:(> ?paxa_head1 ?paxa_head)))
...
;        (retract ?f1 ?f0 ?f4)
        ...
        (bind ?a (sort > ?g))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	R_vi_and_sub	P"?paxa_head"	English_PP	"(implode$	(create$	?R_vi	?a))	")"	crlf)	
        (assert (pada_info (group_head_id ?paxa_head) (group_cat English_PP) (group_ids ?R_vi  ?a)))
 )
...
;Ex. Rama ate apples, a banana and a papaya. 3rd linkage of this sentence is correct. 
 ...
 (declare (salience 501))
 (kriyA-object  ?kriya ?ob)
;(viSeRya-det_viSeRaNa ?ob ?det) ;(not (viSeRya-det_viSeRaNa ?ob ?det)))
 (not (kriyA-object  ?kriya ?ob_id1&:(< ?ob_id1 ?ob)))
 (kriyA-object  ?kriya ?paxa_head)
 (not (kriyA-object  ?kriya ?paxa_head1&:(> ?paxa_head1 ?paxa_head)))
 ?f0<-(to_be_included_in_paxa ?ob)
...
=>
        ...
        (retract ?f ?f0 ?f1)
        (bind $?ids (create$ ))
        (loop-for-count (?loop  ?ob  ?paxa_head)do
                (bind $?ids (insert$ $?ids 1 ?loop))
                (bind $?ids (sort > $?ids))
        )
        (assert (has_been_included_in_paxa $?ids))
        ...
        (assert (count ?count))
        (assert (pada_info (group_head_id ?paxa_head) (group_cat PP) (group_ids  $?ids) ))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	and_obj	P"?paxa_head	"	PP	"(implode$	(create$	$?ids	))	")"crlf)	
 )
...
;Ex. Rama ate an apple, a banana and a papaya. 5th linkage of this sentence is correct. Rama ate my apples, a banana and a papaya. 3rd linkage of this sentence is correct. 
 ...
 (declare (salience 509))
 (kriyA-object  ?kriya ?ob)
 (or (viSeRya-det_viSeRaNa ?ob ?det) (viSeRya-RaRTI_viSeRaNa ?ob ?det))
 (not (kriyA-object  ?kriya ?ob_id1&:(< ?ob_id1 ?ob)))
 (kriyA-object  ?kriya ?paxa_head)
 (not (kriyA-object  ?kriya ?paxa_head1&:(> ?paxa_head1 ?paxa_head)))
 ?f0<-(to_be_included_in_paxa ?ob)
...
=>
        ...
        (assert (count ?count))
        (assert (pada_info (group_head_id ?paxa_head) (group_cat PP) (group_ids  $?ids) ))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	det_and_obj	P"?paxa_head	"	PP	"(implode$	(create$	$?ids	))	")"crlf)	
 )
...
;Ex.  We should teach children how to read write and play.
 ...
 (not (to-infinitive ?to ?inf_id1&:(< ?inf_id1 ?inf)))
 (to-infinitive ?to ?paxa_head)
 (not (to-infinitive ?to ?paxa_head1&:(> ?paxa_head1 ?paxa_head)))
 ?f0<-(to_be_included_in_paxa ?inf)
 ?f1<-(to_be_included_in_paxa ?paxa_head)
 ?f<-(count ?count)
 (test (< ?inf ?paxa_head))
=>
        ...
        (retract ?f ?f0 ?f1)
        (bind $?ids (create$ ))
        (loop-for-count (?loop  ?inf  ?paxa_head)do
                (bind $?ids (insert$ $?ids 1 ?loop))
                (bind $?ids (sort > $?ids))
        )
        (assert (has_been_included_in_paxa $?ids))
        (bind ?count (+ ?count 1))
        (assert (count ?count))
        (assert (pada_info (group_head_id ?paxa_head) (group_cat PP) (group_ids  ?to  $?ids) ))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	and_to_inf	P"?paxa_head	"	PP	"(implode$	(create$	?to	$?ids	))	")"crlf)	
 )
...
;Ex. Most of them have already been noted by various scholars in the course of their investigations on individual languages
 ...
 (link_name-link_expansion ?J  J $?vars)
 (link_name-lnode-rnode   ?J       ?y   ?z)
 ?f0<-(to_be_included_in_paxa ?x) ?f1<-(to_be_included_in_paxa ?paxa_head) ?f2<-(to_be_included_in_paxa ?y) 
 ?f<-(count ?count)
...
=>
        ...
        (assert (has_been_included_in_paxa ?y))
        (assert (has_been_included_in_paxa ?paxa_head))
	(printout	?*dbug*	"(Rule_name-pada_head_id-pada_type-ids	Mp_J	P"?paxa_head	"	PP	"?x"	"	?y"	"	?paxa_head")"crlf)	
        (assert (pada_info (group_head_id ?paxa_head) (group_cat PP) (group_ids  ?x ?y ?paxa_head)))
)
