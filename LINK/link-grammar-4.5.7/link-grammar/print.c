...
//##################################################################################################################
{
	...
	for (j=d; j<N_words_to_print; j++) {
		...
		append_string(string, "(%s)", linkage->word[j]);
                ...
                *  replacing the word "?" with question_mark , "(" with open_brace and ")" with close_brace*/
               ...
               if(( p=strchr(linkage->word[j],' '))!=NULL)
                 { *p= '\0';fprintf(fp1, "(linkid-word\tL%d\t%s)\n",j,linkage->word[j]);
                    fprintf(fp2, "(link_numeric_id-word\t%d\t%s)\n",j,linkage->word[j]); }
